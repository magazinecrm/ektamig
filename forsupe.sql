...
select  * from temp_subvep where (mflag is  null) and name is  not null  order by id;
...
begin
            ...
            loop
               warn('zzzzz');
                msubscriber_no  := find_sno(xxx.name);  
...
*/          
  ...
  (paid_date,id,mdesc,manual_pid,cust_Type,ctn_no,postal,ono,sub_status,membership_number,tran_type,subscriber_no,subscriber_type,company,fullname,res_address1,
                     ...
                     values 
(to_date(xxx.receipt_date,'yyyy-mm-dd'),xxx.id,'expiredpluto',xxx.SUB__REC__BILL_NO,mcust_Type,mctn_no,mpostal,null,'ACTIVE','axmno',null,msubscriber_no,'PAID',mCOMPANY,mfullname,mres_add1,
